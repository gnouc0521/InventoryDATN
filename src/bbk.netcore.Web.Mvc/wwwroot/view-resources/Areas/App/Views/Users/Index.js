(function () {
    $(function () {
        ...
        var dataTable = _$usersTable.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    orderable: false,
                    autoWidth: false,
                    //defaultContent:'',rowAction:{text:'<i class="fa fa-cog"></i> '+app.localize('Actions')+' <span class="caret"></span>',items:[{text:app.localize('LoginAsThisUser'),visible:function(data){return _permissions.impersonation&&data.record.id!==abp.session.userId;},action:function(data){abp.ajax({url:abp.appPath+'Account/Impersonate',data:JSON.stringify({tenantId:abp.session.tenantId,userId:data.record.id})});}},{text:app.localize('Edit'),visible:function(){return _permissions.edit;},action:function(data){_createOrEditModal.open({id:data.record.id});}},{text:app.localize('Permissions'),visible:function(){return _permissions.changePermissions;},action:function(data){_userPermissionsModal.open({id:data.record.id});}},{text:app.localize('Unlock'),visible:function(){return _permissions.changePermissions;},action:function(data){_userService.unlockUser({id:data.record.id}).done(function(){abp.notify.success(app.localize('UnlockedTheUser',data.record.userName));});}},{text:app.localize('Delete'),visible:function(){return _permissions.delete;},action:function(data){deleteUser(data.record);}}]},
                    render: function (data, type, row, meta) {
                        //console.log(data, type, row, meta);
                        //return '<div class="input-group-append"><button type="button" class="btn btn-success shadow-0 dropdown-toggle dropdown-toggle-split waves-effect waves-themed" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Lựa chọn <span class="sr-only">Toggle Dropdown</span>                                                        </button>                                                        <div class="dropdown-menu dropdown-menu-right" x-placement="top-end" style="position: absolute; will-change: top, left; top: -199px; left: 519px;">                                                            <a class="dropdown-item" href="#">Action</a>                                                            <a class="dropdown-item" href="#">Another action</a>                                                            <a class="dropdown-item" href="#">Something else here</a>                                                            <div role="separator" class="dropdown-divider"></div>                                                            <a class="dropdown-item" href="#">Separated link</a>                                                        </div>                                                    </div >';

                        // actions
                        //<a class='dropdown-item' href='javascript:void(0);'>Permissions</a>
                        //<a class='dropdown-item' href='javascript:void(0);'>Unlock</a>
                        return `
                            <div class='dropdown d-inline-block'>
                            	...
                            	<div class='dropdown-menu'>
                            		<a class='dropdown-item usereditfunc' data-userid='` + row.id + `' href='javascript:void(0);'>` + app.localize('Cập nhật') + `</a>
                            		<a class='dropdown-item userpermissionfunc' data-userid='` + row.id + `' href='javascript:void(0);'>` + app.localize('Phân quyền') + `</a>
                            	</div>
                            </div>`;
                    },
                    rowAction: {
        ...
        $('#usersTbl').on('click', '.usereditfunc', function (e) {
            var btnClick = $(this);
            _createOrEditModal.open({ id: btnClick[0].dataset.userid });
        });
        $('#usersTbl').on('click', '.userpermissionfunc', function (e) {
            var btnClick = $(this);
            _userPermissionsModal.open({ id: btnClick[0].dataset.userid });
        });
        $('#usersTbl').on('click', '.userdeletefunc', function (e) {
            var btnClick = $(this);
            //_createOrEditModal.open({ id: btnClick[0].dataset.userid });
            //deleteUser
        });
