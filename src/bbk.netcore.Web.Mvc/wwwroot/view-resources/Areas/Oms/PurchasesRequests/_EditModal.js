(function ($) {
    app.modals.PurchasesRequestEditModal = function () {
        ...
        this.init = function (modalManager) {
            ...
            $('#SubsidiaryCompanyId').val($('#SubsidiaryCompanyId_hidden').val())
            ...
            var getFilter = function () {
                ...
                dataFilter.subsidiaryId = $('#SubsidiaryCompanyId_hidden').val();
                dataFilter.subsidiaryCompanyId = $('#SubsidiaryCompanyId_hidden').val();
                return dataFilter;
            }
            ...
            _itemsServiceService.getItemList().done(function (result) {
                ...
                _purchasesRequestsdetail.getAll(getFilter()).done(function (result) {
                    $.each(result.items, function (index, value) {
                    ...
                    _itemsServiceService.getItemImportList().done(function (result1) {
                        _SupplierService.getSupplierList().done(function (results) {
                            _unitService.getUnitList().done(function (unit) {
                                ...
                                $.each($('.selectSupplier'), function (index1, value1) {
                                    var sel = $('.selectSupplier');
                                    $.each(results, function (index, value) {
                                        abc = `<option value=` + value.id + `>` + value.code + `</option>`
                                        $(sel[index1]).children('option:last').after(abc)
                                    })
                                })
                                ...
                                $.each(result.items, function (index, value) {
                                    ...
                                    $(tr[index]).children('th').find('.SupplierId').val(value.supplierId);
                                    $(tr[index]).children('th').find('.UnitId').val(value.unitId);
                                    $(tr[index]).children('th').find('.Quantity').val(value.quantity);
                                    $(tr[index]).children('th').find('.Uses').val(value.uses);
                                    $(tr[index]).children('th').find('.requestDate').val(moment(value.requestDate).format('L'));
                                    $(tr[index]).children('th').find('.Note').val(value.note);
                                })
                            })
                        })
                    })
                    ...
                    _itemsServiceService.getItemImportList().done(function (result) {
                        _SupplierService.getSupplierList().done(function (results) {
                            ...
                            $('.selectSupplier').select2({
                                width: "100%",
                                dropdownParent: $('#PurchesRequestsEditModal'),
        ...
        this.save = function () {
            ...
            if (length == 0) {
                ...
                _purchasesRequestService.update(data)
                    .done(function (result) {
                        ...
                        $('#ItemTable tbody tr').each(function (index, value) {
                            ...
                            if (data.Id != undefined) {
                                ...
                                _purchasesRequestsdetail.update(data)
                            } else {
                        ...
                        function myFunction(value, index, array) {
                            _purchasesRequestsdetail.delete(value)
                        }
                    }).always(function () {
